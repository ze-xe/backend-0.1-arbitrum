...
export async function getLimitMatchedOrders(req: any, res: any) {
    ...
    try {
        ...
        if (orderType == ORDER_TYPE.BUY) {
            let sellOrder: any | ifOrderCreated[] = Order.find({ pair: pairId, token0: isPairIdExist.token1, pairPrice: { $lte: Number(price) }, chainId: chainId, action: { $in: [0, 2] }, deleted: false, active: true, cancelled: false }).sort({ pairPrice: 1 }).collation({ locale: "en_US", numericOrdering: true }).lean();
            let sellOrder1: any | ifOrderCreated[] = Order.find({ pair: pairId, token0: isPairIdExist.token0, pairPrice: { $lte: Number(price) }, chainId: chainId, action: 1, deleted: false, active: true, cancelled: false }).sort({ pairPrice: 1 }).collation({ locale: "en_US", numericOrdering: true }).lean();
