...
export async function _handleMarginOrderCreated(signature: string, data: any, chainId: string, ipfs: boolean ) {
    try {
        ...
        for (let i in addresses) {
            ...
            if (!ethers.utils.isAddress(addresses[i])) {
                console.log(`${errorMessage.address, addresses[i]}`);
                return { status: false, error: errorMessage.address, statusCode: 400 };
            }
        }
        // geting exchangeRate decimal
        let exchangeRateDecimals: number | string = Number(getDecimals(data.exchangeRate));
        ...
        if (isNaN(exchangeRateDecimals) == true) {
            return { status: false, error: exchangeRateDecimals, statusCode: 400 };
        }
        ...
        if (!id) {
            ...
            return { status: false, error: errorMessage.signature , statusCode: 400};
        }

        const isOrderPresent = await OrderCreated.findOne({ id: id, chainId: chainId }).lean();

        if (isOrderPresent) {
            return { status: true, message: "marginOrder Already Created" , statusCode: 201};
        }
